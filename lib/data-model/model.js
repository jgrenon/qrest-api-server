...
Model.prototype.wrap = function(data, options = {strip: true, private: false }) {
    ...
    var result = Joi.validate(data, this.schema, { stripUnknown: options.strip });
    if(!result.error) {
...
Model.prototype.unwrap = function() {
    if(this.schema) {
        var result = Joi.validate(this, this.schema, { stripUnknown: true, allowUnknown: true });
        if(!result.error) {
            return result.value;
        }
        else {
        ...
        console.log("No schema was found for model", this.key);
        return this;
    }
};
